...
/** Author i18n declaration preserved across re-renders (set by {@link captureLangDecl}). */
export const CORE_LANG_DECL_ATTR = 'data-core-lang-decl';
...
export class Core_HTMLElement extends HTMLElement {
   ...
   #uiAbort;         // Aborts listeners registered via bindUI / bindDelegated during the current render cycle
   /** @type {string|null} */
   #langDecl;        // Author i18n declaration preserved across re-renders
   ...
   /**
    * Preserves template {@code data-core-lang} in {@link #langDecl} and removes it from the host.
    */
   #stashLangDecl() {
      const fromAttr = this.getAttribute('data-core-lang');
      if (fromAttr) {
         this.#langDecl = fromAttr;
         this.removeAttribute('data-core-lang');
      }
   }

   /**
    * Cleans up memory and event listeners related to the UI.
    * Aborts listeners registered via {@link bindUI} / {@link bindDelegated}.
    * Child classes must call `super.cleanFunctional()` when overriding.
    */
   cleanFunctional() {
      if (this.#uiAbort) {
         this.#uiAbort.abort();
   ...
   /**
    * Returns the AbortSignal for the current render-cycle UI bindings.
    * @returns {AbortSignal}
    */
   #getUiSignal() {
      if (!this.#uiAbort) {
      ...
      return this.#uiAbort.signal;
   }

   /**
    * Registers a listener on this element for the current render cycle.
    * Cleaned automatically on the next {@link render} or disconnect.
    *
    * @param {string} type Event type.
    * @param {EventListenerOrEventListenerObject} handler Event handler.
    * @param {AddEventListenerOptions} [options] Optional listener options (signal is managed internally).
    */
   bindUI(type, handler, options = {}) {
      const { signal: _ignored, ...rest } = options;
      this.addEventListener(type, handler, { ...rest, signal: this.#getUiSignal() });
   }

   /**
    * Registers a delegated listener on this element for the current render cycle.
    * Survives child re-renders as long as matching nodes remain under this host.
    * Cleaned automatically on the next {@link render} or disconnect.
    *
    * @param {string} type Event type (including bubbling custom events).
    * @param {string} selector CSS selector matched via `event.target.closest(selector)`.
    * @param {(event: Event, match: Element) => void} handler Called with the event and matched element.
    * @param {AddEventListenerOptions} [options] Optional listener options (signal is managed internally).
    */
    ...
    */
   /**
    * Clears the component DOM. Does not inject content — subclasses set trusted markup in ui_render().
    */
   ui_render() {
      this.innerHTML = '';
   }
