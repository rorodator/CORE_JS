import { Core_HTMLElement } from '../base/core-html-element.js';

/**
 * Contains everything that's required to manage a local Router
 */
export class Core_Router extends HTMLElement {
   /** Document event when a lazy route component fails to load before render. */
   static COMPONENT_LOAD_ERROR_EVENT = 'core-router-component-load-error';
 ...
 */
   addRoute(routeParams) {
      ...
      if ((routeParams['route'])
         && (routeParams['class'] || routeParams['tagName'])) {
   ...
   route(paramURL = null) {
      ...
      // Use the URL passed as parameter, or extract relative path by default
      let url;
      if (paramURL) {
         url = paramURL;
      } else {
         // Get relative path from config service
         url = $svc('config').getRelativePath();
      }
      ...
      this.routes.every((routeDesc, index, array) => {
         // Get the relative route path from config service
         const relativeRoute = $svc('config').getRoute(routeDesc.routeName);
         // Add anchors to ensure exact matching
         const anchoredRoute = '^' + relativeRoute + '$';
         let regExp = new RegExp(anchoredRoute, 'i');


         // We found the proper candidate
         if (matches = regExp.exec(url)) {

            // If we are already on this route with the same match, do nothing but prevent default fallback
            if (routeDesc === this.#currentRoute && matches[0] === this.#lastMatch) {
               routeFound = true;
               return false;
            }

            // New match or different route → proceed
   ...
   processRouteFound(url, routeDesc, params = null) {
      if (routeDesc.tagName) {
         const compSvc = Core_Router.#resolveComponentLoaderService();
         if (compSvc) {
            const tag = routeDesc.tagName;
            compSvc.ensure(tag)
               .then(
                  () => this.#commitRouteFound(url, routeDesc, params),
                  (error) => this.#handleComponentLoadFailure(tag, url, routeDesc, error)
               );
            return;
         }
      }
   ...
   /**
    * Applies a resolved route: cache handling, DOM updates, and routeChanged emission.
    * @param {*} url The URL that matched.
    * @param {*} routeDesc The route descriptor.
    * @param {*} params Optional parameters extracted from the URL.
    */
   #commitRouteFound(url, routeDesc, params = null) {
      ...
      if (this.#currentRoute !== null) {
         ...
         if (kro !== Core_Router.keepRouteOptions.DONT_KEEP) {
            ...
            if ((kro & Core_Router.keepRouteOptions.KEEP_SCROLLS) === Core_Router.keepRouteOptions.KEEP_SCROLLS) {
      ...
      // If a tagName is provided, render via tag (supports lazy load via component loader service)
      if (routeDesc.tagName) {
         const tag = routeDesc.tagName;
         const renderByTag = () => {
            if (routeDesc.memory.object !== null && routeDesc.memory.URL === url) {
               this.#currentChild = routeDesc.memory.object;
               componentReused = true;
            } else {
               const el = document.createElement(tag);
               // Optional hook to pass route params to the element if supported
               if (params && typeof el.setRouteParams === 'function') {
                  try { el.setRouteParams(params); } catch (_) {}
               }
            ...
            // If needed, restore cached attributes/scrolls
            if (componentReused) {
            ...
            // Emit routeChanged event
            const routeChangedEvent = new CustomEvent('routeChanged', {
               detail: { route: routeDesc.route, url: url, params: params }
            });
   ...
   saveChildAttributes(elt) {
      if ((this.#currentRoute.keepRouteOption & Core_Router.keepRouteOptions.KEEP_SCROLLS) === Core_Router.keepRouteOptions.KEEP_SCROLLS
   ...
   */
   restoreChildAttributes(elt) {
      ...
      if (eltData
         && (this.#currentRoute.keepRouteOption & Core_Router.keepRouteOptions.KEEP_SCROLLS) === Core_Router.keepRouteOptions.KEEP_SCROLLS) {
   ...
   /**
    * Logs and emits a platform event when lazy component loading fails.
    * Does not mutate router state — the current view remains unchanged.
    * @param {string} tag Custom element tag that failed to load.
    * @param {string} url Matched route URL.
    * @param {*} routeDesc Route descriptor.
    * @param {*} error Rejection from the component loader.
    */
   #handleComponentLoadFailure(tag, url, routeDesc, error) {
      const normalizedError = error instanceof Error ? error : new Error(String(error));
      const route = routeDesc.routeName ?? routeDesc.route ?? null;

      try {
         $svc('log').error('Router component load failed', {
            tag,
            url,
            route,
      ...
      try {
         document.dispatchEvent(new CustomEvent(
            Core_Router.COMPONENT_LOAD_ERROR_EVENT,
            {
               detail: {
                  tag,
                  url,
   ...
   /**
    * @returns {*|null} Component loader service when registered and usable.
    */
   static #resolveComponentLoaderService() {
      try {
         if (window.$core && typeof $svc === 'function') {
            const compSvc = $svc('components');
            if (compSvc && typeof compSvc.ensure === 'function') {
               return compSvc;
            }
         }
      } catch (_) {}
    ...
    */
   connectedCallback() {
      if (!this.hasAttribute('data-core-isRouter')) {
